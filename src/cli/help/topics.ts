...
export function printHelpTopic(topicRaw: string): void {
  ...
  if (resolved === 'missions') {
    ...
    console.log(`     ${dim('--output ./report.md writes a per-node report (md/json/txt by extension)')}`);
    console.log();
    ...
    console.log();
    console.log(`  ${iColor('3')} ${bright('Pick a plan template via planner.style')}`);
    console.log(`     ${accent('research')}  ${dim('— gather → process → deliver → refine (default for evidence goals)')}`);
    console.log(`     ${accent('qa')}        ${dim('— research-quick → answer (short factual goals, ~5s)')}`);
    console.log(`     ${accent('creative')}  ${dim('— brainstorm → develop → produce → polish (artifact goals)')}`);
    console.log(`     ${dim('Omit')} ${accent('style')} ${dim('and the goal text auto-classifies — leading "what is" / "explain"')}`);
    console.log(`     ${dim('routes to qa, "Write a..." / "Compose..." / "Design a..." routes to creative,')}`);
    console.log(`     ${dim('everything else routes to research. The resolved style is shown in the')}`);
    console.log(`     ${dim('console banner and report header.')}`);
    console.log();
    console.log(`  ${iColor('4')} ${bright('Author a mission YAML')}`);
    console.log(`     ${dim('Minimal structure:')}`);
    console.log(`     ${accent('name: my-mission')}`);
    console.log(`     ${accent('goal: "Research {{topic}} and produce a cited summary"')}`);
    console.log(`     ${accent('planner:')}`);
    console.log(`     ${accent('  strategy: linear')}`);
    console.log(`     ${accent('  maxSteps: 5')}`);
    console.log(`     ${accent('  style: research   # research | qa | creative — omit to auto-classify')}`);
    console.log();
    console.log(`  ${iColor('5')} ${bright('Use mission() in TypeScript')}`);
    ...
    console.log(`     ${accent('examples/mission-deep-research.yaml')}  ${dim('— full 4-step research plan')}`);
    console.log(`     ${accent('examples/mission-qa.yaml')}              ${dim('— short Q&A with style: qa')}`);
    console.log(`     ${accent('examples/mission-creative.yaml')}        ${dim('— write-a-haiku with style: creative')}`);
    console.log();
    return;
  }
  ...
  if (resolved === 'security') {
    ...
    console.log(`     ${accent('--no-guardrail-override')} ${dim('disable post-approval guardrail veto after HITL approval')}`);
