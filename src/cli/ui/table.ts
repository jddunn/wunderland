...
import { dim, accent, muted, bright } from './theme.js';
...
export function renderTable(opts: TableOptions): string {
  ...
  // Header row
  table.push(columns.map((c) => bright(c.label)));
  ...
  if (title) {
    lines.push(`  ${accent('◆')} ${bright(title)}`);
    lines.push('');
  }

  // Indent each line for consistent CLI padding
