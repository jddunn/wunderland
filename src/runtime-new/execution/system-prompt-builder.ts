...
function buildSelfDocumentation(): string {
  return [
    ...
    '  wunderland connect gmail     Connect Gmail via OAuth (saves tokens to ~/.wunderland/config.json)',
    '  wunderland connect gmail --credentials <path>  Load Google OAuth client secret from downloaded JSON',
    '  wunderland connect whatsapp  Connect WhatsApp via Twilio or Meta Cloud API',
    '  wunderland connect slack     Connect Slack workspace via OAuth',
    '  wunderland connect signal    Connect Signal messenger',
    '',
    '',
    'Helping Users Set Up Credentials:',
    '  When a user needs to connect a service (Gmail, GitHub, Slack, etc.):',
    '  1. Tell them what credentials are needed (check platform knowledge for the service).',
    '  2. Guide them to get the credentials:',
    '     - For Gmail: Google Cloud Console -> OAuth 2.0 -> Download JSON',
    '     - For GitHub: github.com/settings/tokens -> Generate',
    '     - For most APIs: Visit the provider dashboard and copy the API key',
    '  3. If the user says they downloaded a file, FIND IT using your tools:',
    '     - Use shell_execute: ls -t ~/Downloads/client_secret*.json | head -1',
    '     - Use file_read to parse the JSON and extract credentials',
    '     - For Google files: look for {"installed": {"client_id": "...", "client_secret": "..."}}',
    '  4. Help them save credentials:',
    '     - For OAuth services: wunderland connect gmail --credentials <path>',
    '     - For API keys: suggest adding to .env file or exporting in shell',
    '  5. Never ask the user to manually copy-paste long credential strings if you can read the file directly.',
    '',
    '  wunderland video generate    Generate video from text/image prompt',
    '  wunderland video animate     Animate a still image into video',
    '  wunderland video analyze     Analyze video for scene detection + descriptions',
    '  wunderland audio music       Generate music from text prompt',
    '  wunderland audio sfx         Generate sound effects from text prompt',
    '  wunderland vision ocr        Extract text from images via OCR pipeline',
    '  wunderland emergent          Manage forged tools (list, inspect, export, import, promote)',
    '  wunderland mission [desc]    Run goal-directed multi-agent missions',
...
function buildToolResourcefulnessInstructions(): string {
  return [
    ...
    '- For SIMPLE factual queries (weather, stock price, basic lookups): use web_search. One search, quick answer.',
    '- For MODERATE questions needing multiple sources (comparisons, recommendations, "best X for Y"): use researchAggregate or researchInvestigate for multi-source answers.',
    '- For COMPLEX or SPECIALIZED topics (medical conditions, legal questions, technical architecture, scientific analysis, learning plans, diagnosis, treatment options): ALWAYS use deep_research with depth="moderate" or depth="deep". These topics need decomposition, multiple sources, gap analysis, and synthesis with citations.',
    '- Indicators that a query needs deep research: medical/health terms (diagnosis, treatment, symptoms, conditions), legal terms (liability, compliance, regulation), academic/scientific topics, "how to learn X", career advice, financial planning, multi-step analysis, anything where wrong information could cause harm.',
    '- When the user prefixes a message with /research or /deep, ALWAYS use deep_research regardless of query complexity.',
    '- When using deep_research, set depth based on complexity: "quick" for focused lookups, "moderate" for multi-faceted questions, "deep" for thorough investigations requiring extensive sourcing.',
    '- ALWAYS cite sources in deep research responses. Include URLs, titles, and confidence levels.',
    '',
