...
function buildSelfDocumentation(): string {
  return [
    ...
    '  If the user wants to connect Gmail or says "use the client secret I downloaded":',
    '  - Run: wunderland connect gmail --credentials ~/Downloads/client_secret_*.json',
    '  - Or just run: wunderland connect gmail (auto-discovers client_secret*.json in ~/Downloads)',
    '  - The Google JSON format is {"installed": {"client_id": "...", "client_secret": "..."}} or {"web": {...}}',
    '  - Extract client_id and client_secret and pass via --credentials or set as env vars',
    '  - Env vars: GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET',
